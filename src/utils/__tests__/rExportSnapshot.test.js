...
import { resolvePointFeatures, resolveRangeFeatures, screenGeometry, snapshotLegend } from '../rExport/snapshot'
...
describe('R export snapshot', () => {
  ...
  it('uses the displayed attribute and visible legend palette, preserving projected point locations', () => {
    for (const [attribute, palette] of [
      ['scientific_name', { A: '#123456', B: '#654321' }],
      ['sequencing_status', { Sequenced: '#123456' }],
      ['mimicry_ring', { Tiger: '#123456' }]
    ]) {
      const out = resolvePointFeatures(records, { attribute, palette, shownLabels: new Set([Object.keys(palette)[0]]), hiddenItems: [], project })
  ...
  it('filters hidden categories before exporting the rendered point source', () => {
    const out = resolvePointFeatures(records, {
      ...
      shownLabels: new Set(['A']), hiddenItems: ['B'], project
    })
