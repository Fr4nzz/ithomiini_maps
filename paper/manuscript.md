**From butterfly photos to interactive maps: source-available tools for AI-assisted photo processing, image identification and distribution mapping**
...
The web interfaces use Vue.js 3 (Vue.js Contributors, n.d.), and Wings Atlas uses MapLibre GL JS for map rendering (MapLibre Contributors, n.d.). Data processing is written primarily in Python (Python Software Foundation, n.d.) with pandas (McKinney, 2010). GitHub Pages hosts the built web applications, and GitHub Actions runs build, deployment and database update workflows (GitHub, n.d.-a, n.d.-b). The AI Photo Processor uses the Gemini application programming interface (API) for image interpretation, with an optional local backend for rotating camera RAW files.
...
![Photography, separate classification workflows and independently versioned Atlas layers.](figures/figure1_workflow.svg)

**Figure 1.** Connections between specimen digitisation and biological data. (A) Photographs with visible specimen identifiers are reviewed and renamed with the AI Photo Processor. (B) Collection photographs are indexed in shared storage and displayed in Wings Gallery. (C) Uploaded photographs use a separate inference service that returns taxonomic candidates. (D) Paired dorsal and ventral collection photographs use a distinct fitted classifier and release. (E) Wings Atlas combines occurrence sources with taxonomic curation, mimicry assignments, recorded genomic-sampling status, host-plant information and relative habitat-suitability layers. Occurrence records, independent plant occurrences and model predictions are distinct data types. (F) The occurrence snapshot, species distribution model (SDM) products and classifier releases have separate version identifiers.
...
The three applications are connected by a photography-to-gallery workflow. The project setup uses a Canon EOS 550D and a Canon EF-S 18–55 mm f/4–5.6 IS STM lens positioned approximately 28 cm above the table. This is an example setup, not a required camera specification. Camera position, framing, lighting and exposure should be adjusted for the lens and specimen size and kept consistent within a session.
...
Renamed files are uploaded to shared Google Drive folders, indexed through Google Apps Script and Google Sheets, processed with Python scripts, and served through the Wings Gallery. The camera setup, image review, file validation and indexing steps are described in [Supplementary Protocol S1](supplementary_protocol_S1.md).
...
The Atlas distinguishes five occurrence sources: the published dataset of Doré et al. (2023), Sanger Institute collection records, and three GBIF-derived groups comprising iNaturalist research-grade observations, Universidad Nacional Autónoma de México (UNAM) records and other institutional records (Table 1). The Doré et al. dataset provides occurrence information and sex-specific mimicry-ring assignments. The GBIF groups retain their source identities so that museum and citizen-science records can be compared separately. Project documentation records `10.15468/dl.6zagkz` as the butterfly GBIF download identifier. Its registered download metadata remain unconfirmed, so the identifier alone does not establish the scope of that component.
...
The processing pipeline standardises field names, country names and taxonomic formatting across sources. Taxonomic curation uses a manually maintained corrections file, the GBIF backbone taxonomy, and reference names from Butterflies of America (Warren et al., 2024) and Nymphalidae.net (Wahlberg, n.d.). The workflow addresses synonyms, misspellings and outdated names, with edit-distance checks to flag possible typographical errors. Original names are retained alongside curated names and the recorded basis for the change. A resolved name does not by itself validate the identification of the underlying specimen.
...
Occurrence records are filtered or flagged according to the evidence represented by each source. GBIF-derived records are screened for four coordinate-quality problems before use in the web-map dataset and species distribution models: reported coordinate uncertainty above 100 km, the GBIF placeholder locality string "no specific locality", coordinates outside the Neotropical study extent (120°W to 30°W, 40°S to 25°N), and coordinates falling in the ocean. The configured ocean check uses the Global Self-consistent, Hierarchical, High-resolution Shoreline database (GSHHS, Wessel & Smith, 1996) with a 5 km coastal tolerance. This check depends on the available land-mask assets. The implementation can fall back to a coarser mask or omit ocean filtering when no usable mask is available.
...
Larval host records are scattered across the literature and can differ in both taxonomic precision and evidential support. The compilation draws on the Neotropical host-plant catalogue of Beccaloni et al. (2008), the phylogenetic synthesis of Willmott & Freitas (2006) and primary association reports, including Costa (1999) and McClure & Elias (2016). The host-plant module links butterfly taxa to reported larval hosts, retaining the reported plant name, accepted name, family, source citation and accompanying notes. The reported name preserves what the source identified. The accepted name facilitates comparison across synonyms, without adding identification precision absent from the source.
...
The modelling pipeline uses occurrence records from GBIF-derived sources after source-specific spatial filtering and 5 km spatial thinning. The environmental downloader specifies CHELSA v2.1 bioclimatic variables BIO1, BIO2, BIO4, BIO5, BIO6, BIO12, BIO13, BIO14 and BIO15 for the 1981–2010 baseline at 30-arc-second resolution (Karger et al., 2021). Elevation is obtained from the 30-arc-second WorldClim v2.1 elevation product, derived from the Shuttle Radar Topography Mission (Fick & Hijmans, 2017). Mean annual cloud frequency comes from the EarthEnv Global 1-km Cloud Frequency dataset, version 1 (Wilson & Jetz, 2016). These source resolutions are distinct from the prediction grid. Host-plant information is not included in this SDM predictor stack. The plant layers are intended for ecological context and hypothesis generation, not as predictors in the cross-species modelling workflow, because host records and GBIF plant occurrence availability are uneven across taxa.
...
The modelling algorithm depends on sample size. Species with 20–49 thinned records are fitted with MaxEnt alone (Phillips & Dudík, 2008) through the elapid Python package (Anderson, 2023). Species with at least 50 records use an ensemble of MaxEnt, random forest (Breiman, 2001) and gradient-boosted trees implemented with XGBoost (Chen & Guestrin, 2016). This sample-size-dependent approach reflects differences in model performance across species and available records (Wisz et al., 2008, Valavi et al., 2022).

For species with at least 50 records, the pipeline uses spatial-block cross-validation where feasible (Roberts et al., 2017). The 20–49-record tier generates leave-one-out predictions, an approach used for small occurrence samples (Pearson et al., 2007). In this tier, the implemented area under the receiver operating characteristic curve (AUC) and continuous Boyce summaries are calculated from a model fitted to all available records, not from the held-out predictions. These small-sample summaries therefore cannot be interpreted as held-out validation. The continuous Boyce index assesses whether observed presences are concentrated in areas of higher predicted suitability (Hirzel et al., 2006).

MaxEnt feature classes and regularisation are compared using continuous Boyce diagnostics, following the tuning framework described by Kass et al. (2021). The workflow compares candidate and default configurations and favours simpler models when scores are similar. These diagnostics inherit the validation design of the sample-size tier, including the full-fit limitation for small samples. They are used for model selection and are not an independent assessment of the selected model.
...
**Table 1.** Data sources represented in the frozen 9 May 2026 Wings Atlas occurrence snapshot, comprising 104,297 records in total.
...
Source code for the [AI Photo Processor](https://github.com/Fr4nzz/rename_photos_AI) is available at commit `49fe6ed226ed4914ed793e4252b74e18cf6ad8a4`, with a [browser application](https://fr4nzz.github.io/rename_photos_AI/). [Wings Gallery source code](https://github.com/rapidspeciation/Shiny_Ikiam_Wings_Gallery) accompanies its [public interface](https://rapidspeciation.github.io/Shiny_Ikiam_Wings_Gallery/). Uploaded-photo classification is accessed through the AI Identifier tab, whose application route is `ai_identifier`. The [Wings Atlas application](https://rapidspeciation.github.io/ithomiini_maps/) is published from the [Atlas repository](https://github.com/rapidspeciation/ithomiini_maps). The [working manuscript repository](https://github.com/Fr4nzz/ithomiini_maps) contains the manuscript, workflow figure, Supplementary Protocol S1 and submission provenance records. Source availability, deployment and permission to reuse materials are separate.

The underlying Atlas source state is identified by commit `935cc42740aefbe70640eb15d79bf4f6c21e1089`. The occurrence manifest dates the selected snapshot to 9 May 2026. Project documentation records butterfly GBIF download identifier `10.15468/dl.6zagkz`, but its registered metadata could not be confirmed. No later download is substituted for this component. The complete SDM raster release is identified by commit `1cdde9bc8d5f4375def888b5796e71493ee60d68` of 28 April 2026. Generated occurrence, SDM and host-plant display files are served with the Atlas. These are distinct from restricted source spreadsheets, and the host files are not presented as a reconciled quantitative release. No plant GBIF DOI is assigned to the aggregate of exported host layers.

The Gallery methods and published evaluation are tied to commit `f5cb03f17f3ba639090b32fbedde6d6645ff847b`. Its paired prediction coverage receipt is dated 14 September 2026 and records classifier SHA-256 `34ecd53f64b3f9dc600710af5ca8df7cd94845653a9ce9f916dcd08adc520167`. [Uploaded-photo inference source](https://huggingface.co/spaces/fr4nzzch/butterfly-id) is available at commit `3aa72a33946f2da661d1ed96f0929e92cd9413e6`, with the [inference service](https://fr4nzzch-butterfly-id.hf.space) deployed separately. [Published classifier resources](https://huggingface.co/fr4nzzch/butterfly-id-classifier) include the uploaded-photo classifier object with SHA-256 `e461eb4eda24c919711d7497833916305259a712753323ff9431aad9b9e07d6f6`. The [BioCLIP 2.5-H model resource](https://huggingface.co/imageomics/bioclip-2.5-vith14), cited separately from the BioCLIP 2 paper, is identified by resource revision `6e3d04e3d6522012c88181085c5ae666e14c45cd` (Gu et al., 2026). These published identifiers do not independently attest which weights a running container has loaded.

Restricted Sanger collection information is available upon reasonable request to the research group, subject to project permissions and specimen-data sensitivity. Source-code licensing must be checked for each component. The BioCLIP model resource declares an MIT licence, whereas the project's published classifier repository declares CC BY-NC 4.0. Third-party occurrence records, environmental data and images retain their source licences and attribution requirements. Availability of code or classifier weights does not confer redistribution rights to training, evaluation or reference images.

# 7. Acknowledgements

We thank Neil Rosser and James Mallet for developing the *Heliconius* maps platform, which inspired the design of Wings Atlas. We acknowledge GBIF and its data publishers for providing occurrence data. AI assistants were used during software development and manuscript preparation, including OpenAI GPT models, Google Gemini and Anthropic Claude.
...
Beccaloni, G.W., Viloria, Á.L., Hall, S.K. & Robinson, G.S. (2008). *Catalogue of the hostplants of the Neotropical butterflies / Catálogo de las plantas huésped de las mariposas neotropicales*. Monografías Tercer Milenio, 8. Sociedad Entomológica Aragonesa, Zaragoza. 536 pp. <https://ci.nii.ac.jp/ncid/BB00704591>
...
Costa, F.A.P.L. (1999). New records of larval hostplants for Ithomiinae butterflies (Nymphalidae). *Revista Brasileira de Biologia*, 59(3), 455–459. <https://doi.org/10.1590/S0034-71081999000300010>

Doré, M., Willmott, K., Lavergne, S., Chazot, N., Freitas, A.V.L., Fontaine, C. & Elias, M. (2023). Mutualistic interactions shape global spatial congruence and climatic niche evolution in Neotropical mimetic butterflies. *Ecology Letters*, 26(6), 843–857. <https://doi.org/10.1111/ele.14198>

Elias, M., Gompert, Z., Jiggins, C. & Willmott, K. (2008). Mutualistic interactions drive ecological niche convergence in a diverse butterfly community. *PLoS Biology*, 6(12), e300. <https://doi.org/10.1371/journal.pbio.0060300>
...
GitHub (n.d.-a). *GitHub Actions documentation*. <https://docs.github.com/en/actions>. Accessed 22 September 2026.

GitHub (n.d.-b). *GitHub Pages documentation*. <https://docs.github.com/en/pages>. Accessed 22 September 2026.

Gu, J., Stevens, S., Campolongo, E.G., Thompson, M.J., Zhang, N., Wu, J., Kopanev, A., Mai, Z., White, A.E., Balhoff, J., Dahdul, W., Rubenstein, D., Lapp, H., Berger-Wolf, T., Chao, W.-L. & Su, Y. (2025). BioCLIP 2: Emergent properties from scaling hierarchical contrastive learning. *Advances in Neural Information Processing Systems*, 38, 102778–102811. <https://doi.org/10.52202/085713-3436>

Gu, J., Stevens, S., Campolongo, E.G., Thompson, M.J., Zhang, N., Wu, J., Kopanev, A., Mai, Z., White, A.E., Balhoff, J., Dahdul, W.M., Rubenstein, D., Lapp, H., Berger-Wolf, T., Chao, W.-L. & Su, Y. (2026). *BioCLIP 2.5 Huge* (version 1.0.0) [Model]. Hugging Face. <https://doi.org/10.57967/hf/10131>
...
Jocher, G., Qiu, J. & Chaurasia, A. (2023). *Ultralytics YOLO* [Software]. <https://github.com/ultralytics/ultralytics>. Accessed 22 September 2026.

Karger, D.N., Conrad, O., Böhner, J., Kawohl, T., Kreft, H., Soria-Auza, R.W., Zimmermann, N.E., Linder, H.P. & Kessler, M. (2021). *Climatologies at high resolution for the earth’s land surface areas* (version 2.1) [Dataset]. EnviDat. <https://doi.org/10.16904/envidat.228>
...
MapLibre Contributors (n.d.). *MapLibre GL JS documentation*. <https://maplibre.org/maplibre-gl-js/docs/>. Accessed 22 September 2026.

McClure, M. & Elias, M. (2016). Unravelling the role of host plant expansion in the diversification of a Neotropical butterfly genus. *BMC Evolutionary Biology*, 16, 128. <https://doi.org/10.1186/s12862-016-0701-5>
...
Python Software Foundation (n.d.). *The Python language reference*. <https://docs.python.org/3/reference/index.html>. Accessed 22 September 2026.

Roberts, D.R., Bahn, V., Ciuti, S., Boyce, M.S., Elith, J., Guillera-Arroita, G., Hauenstein, S., Lahoz-Monfort, J.J., Schröder, B., Thuiller, W., Warton, D.I., Wintle, B.A., Hartig, F. & Dormann, C.F. (2017). Cross-validation strategies for data with temporal, spatial, hierarchical, or phylogenetic structure. *Ecography*, 40(8), 913–929. <https://doi.org/10.1111/ecog.02881>

Rosser, N. & Mallet, J. (2024). Interactive maps for visualizing geographic distributions and phenotypes. *Tropical Lepidoptera Research*, 34(2), 104–107. <https://doi.org/10.5281/zenodo.13920055>
...
Vue.js Contributors (n.d.). *Vue.js 3 documentation*. <https://vuejs.org/guide/introduction.html>. Accessed 22 September 2026.

Wahlberg, N. (n.d.). *Nymphalidae.net*. <https://www.nymphalidae.net/>. Accessed 22 September 2026.

Warren, A.D., Davis, K.J., Stangeland, E.M., Willmott, K.R. & Grishin, N.V. (2024). *Illustrated lists of American butterflies* (version 9 March 2024). Butterflies of America Foundation. <https://www.butterfliesofamerica.com/>. Accessed 22 September 2026.
...
Willmott, K.R. & Freitas, A.V.L. (2006). Higher-level phylogeny of the Ithomiinae (Lepidoptera: Nymphalidae): classification, patterns of larval hostplant colonization and diversification. *Cladistics*, 22(4), 297–368. <https://doi.org/10.1111/j.1096-0031.2006.00108.x>
