...
Ithomiini butterflies (Nymphalidae: Danainae) are a useful study system because they combine large specimen collections with questions that depend on geography, phenotype and genomic sampling. Distributed across Central and South America, they show extensive variation in larval host associations and geographic distributions (Willmott & Freitas, 2006, Chazot et al., 2016). Many species combine warning colouration with wing transparency (McClure et al., 2019). Their wing colour patterns are shaped by Müllerian mimicry among co-occurring species, linking phenotypic variation to ecological interactions and biogeography (Elias et al., 2008, Doré et al., 2023). Published occurrence records with mimicry-ring assignments, together with ongoing collections, provide a basis for comparing these patterns across regions. Connecting specimens and their photographs to genomic resources also helps researchers select material for studies of diversification and the genetic basis of phenotypic variation (Gauthier et al., 2023, van der Heijden et al., 2025, Ben Chehida et al., 2026).
...
The web interfaces use Vue.js 3 (You, n.d.), and Wings Atlas uses MapLibre GL JS for map rendering (MapLibre Contributors, n.d.). Data processing is written primarily in Python (Python Software Foundation, n.d.) with pandas (McKinney, 2010). GitHub Pages hosts the built web applications (GitHub, n.d.-b), and GitHub Actions runs build, deployment and database update workflows (GitHub, n.d.-a). The AI Photo Processor uses the Gemini API for image interpretation, with an optional local backend for RAW-image rotations.
...
The three applications are connected by a reproducible photography-to-gallery workflow. For the Ithomiini workflow, wings are photographed with a stable camera setup using a Canon EOS 550D and a Canon EF-S 18-55 mm f/4-5.6 IS STM lens. The lens is positioned approximately 28 cm above the table, and the zoom position is taped to reduce accidental changes between imaging sessions. The archived setup record documents an example image at 33 mm focal length, 1/100 s exposure, f/8, ISO 100 and manual exposure mode. These values describe the setup used here and should be adjusted for other cameras, lenses, lighting conditions, or specimen sizes.
...
Renamed files are uploaded to shared Google Drive folders, indexed through Google Apps Script and Google Sheets, processed with Python scripts, and served through the Wings Gallery. The camera setup, image review and indexing steps are described in [Supplementary Protocol S1](supplementary_protocol_S1.md).
...
Larval host records are scattered across the literature and can differ in both taxonomic precision and evidential support. The compilation draws on host catalogues and published biological records, including Beccaloni et al. (2008), Costa (1999) and Willmott & Freitas (2006). The host-plant module links butterfly taxa to reported larval hosts, retaining the reported plant name, accepted name, family, source citation and accompanying notes. The reported name preserves what the source identified. The accepted name facilitates comparison across synonyms, without adding identification precision absent from the source.
...
The modelling pipeline uses occurrence records from GBIF-derived sources after source-specific spatial filtering and 5 km spatial thinning. The environmental-data downloader specifies CHELSA v2.1 bioclimatic variables for 1981–2010 at 30 arc-second resolution (BIO1, BIO2, BIO4, BIO5, BIO6, BIO12, BIO13, BIO14 and BIO15, Karger et al., 2021), the SRTM-derived elevation layer distributed with WorldClim v2.1 at 30 arc seconds (`wc2.1_30s_elev`, Fick & Hijmans, 2017), and the mean annual layer from EarthEnv Global 1-km Cloud Frequency version 1 (`MODCF_meanannual.tif`, Wilson & Jetz, 2016). The last predictor measures MODIS-derived cloud frequency. Host-plant information is not included in this SDM predictor stack. The plant layers are intended for ecological context and hypothesis generation, not as predictors in the cross-species modelling workflow, because host records and GBIF plant occurrence availability are uneven across taxa.
...
The modelling algorithm depends on sample size. Species with 20 to 49 thinned records are fitted with MaxEnt alone (Phillips & Dudík, 2008) through the elapid Python package (Anderson, 2023). Species with 50 or more records use an ensemble of MaxEnt, random forest, and gradient-boosted trees, following evidence that different presence-only algorithms can perform differently across species and sample sizes (Wisz et al., 2008, Valavi et al., 2022). Models are evaluated with spatial-block cross-validation where sample size permits, and leave-one-out cross-validation is used for species with fewer than 30 records (Pearson et al., 2007, Roberts et al., 2017). Evaluation reports the area under the receiver operating characteristic curve (AUC) and the continuous Boyce index, which measures whether higher predicted suitability contains a higher density of observed presences (Hirzel et al., 2006).
...
The AI Photo Processor source code is available at <https://github.com/Fr4nzz/rename_photos_AI>, with its browser interface at <https://fr4nzz.github.io/rename_photos_AI/>. Supplementary Protocol S1 documents the workflow against source commit `49fe6ed226ed4914ed793e4252b74e18cf6ad8a4`. Wings Gallery source code is available at <https://github.com/rapidspeciation/Shiny_Ikiam_Wings_Gallery>, and its public interface is at <https://rapidspeciation.github.io/Shiny_Ikiam_Wings_Gallery/>. Uploaded-photo identification is accessed through <https://rapidspeciation.github.io/Shiny_Ikiam_Wings_Gallery/ai_identifier>. The Gallery methods and published collection evaluation refer to source commit `f5cb03f17f3ba639090b32fbedde6d6645ff847b`.

Wings Atlas is available at <https://rapidspeciation.github.io/ithomiini_maps/>. Its working repository, including the manuscript and Supplementary Protocol S1 in `paper/`, is <https://github.com/Fr4nzz/ithomiini_maps>. The application and included data underlying the manuscript are identified by commit `935cc42740aefbe70640eb15d79bf4f6c21e1089`. The complete 28 April 2026 SDM raster release is identified separately by commit `1cdde9bc8d5f4375def888b5796e71493ee60d68`. Generated occurrence display files, SDM products and host-plant files are distributed with the Atlas. These outputs are distinct from restricted source spreadsheets and do not constitute a newly reconciled quantitative host-plant release. The source downloads used for the occurrence snapshot are described in Section 2.5.

Uploaded-photo inference source is available at <https://huggingface.co/spaces/fr4nzzch/butterfly-id>, inspected at commit `3aa72a36c37ce8b383f8f5a5f9ec5763f52e5e42`. The deployed service is <https://fr4nzzch-butterfly-id.hf.space>. Model assets are distributed at <https://huggingface.co/fr4nzzch/butterfly-id-classifier>, separately from the upstream BioCLIP 2.5-H resource at <https://huggingface.co/imageomics/bioclip-2.5-vith14>. The published uploaded-photo classifier object has SHA-256 `e461eb4eda24c919711d7497833916305259a712753323ff9431aad9b9e07d6f6`. The distinct paired-collection classifier is identified in the 14 September 2026 prediction coverage receipt by SHA-256 `34ecd53f64b3f9dc600710af5ca8df7cd94845653a9ce9f916dcd08adc520167`. Source revisions and published object hashes do not independently attest which weights a running service has loaded.

These Git commits identify immutable source versions, while the public interfaces and external storage may change. Restricted Sanger collection information is available upon reasonable request to the corresponding research group, subject to project permissions and specimen-data sensitivity. Third-party records and images retain their source licences and attribution requirements, including images used for training, evaluation and reference displays. Public code or model assets do not confer redistribution rights for all underlying images. No plant GBIF DOI is assigned to the unreconciled aggregate of host layers.
...
Barve, N., Barve, V., Jiménez-Valverde, A., Lira-Noriega, A., Maher, S.P., Peterson, A.T., et al. (2011). The crucial role of the accessible area in ecological niche modeling and species distribution modeling. Ecological Modelling, 222(11), 1810–1819. <https://doi.org/10.1016/j.ecolmodel.2011.02.011>.

Beccaloni, G.W., Viloria, Á.L., Hall, S.K. & Robinson, G.S. (2008). Catalogue of the hostplants of the Neotropical butterflies / Catálogo de las plantas huésped de las mariposas neotropicales. Monografías Tercer Milenio, 8. Sociedad Entomológica Aragonesa, Zaragoza. 536 pp. ISBN 978-84-935872-2-2. <https://www.researchgate.net/publication/262099000_Catalogue_of_the_Hostplants_of_the_Neotropical_Butterflies_Catalogo_de_las_Plantas_Huesped_de_las_Mariposas_Neotropicales>.

Ben Chehida, Y., van der Heijden, E.S.M., Page, E., Salazar C, P.A., Rosser, N., Córdova, K.G.G., et al. (2026). Genetic parallelism underpins convergent mimicry coloration in Lepidoptera across 120 million years of evolution. PLoS Biology, 24(4), e3003742. <https://doi.org/10.1371/journal.pbio.3003742>.
...
Chazot, N., Willmott, K.R., Condamine, F.L., De-Silva, D.L., Freitas, A.V.L., Lamas, G., et al. (2016). Into the Andes: multiple independent colonizations drive montane diversity in the Neotropical clearwing butterflies Godyridina. Molecular Ecology, 25(22), 5765–5784. <https://doi.org/10.1111/mec.13773>.
...
Doré, M., Willmott, K., Lavergne, S., Chazot, N., Freitas, A.V.L., Fontaine, C., et al. (2023). Mutualistic interactions shape global spatial congruence and climatic niche evolution in Neotropical mimetic butterflies. Ecology Letters, 26(6), 843–857. <https://doi.org/10.1111/ele.14198>.
...
Gauthier, J., Meier, J., Legeai, F., McClure, M., Whibley, A., Bretaudeau, A., et al. (2023). First chromosome scale genomes of ithomiine butterflies (Nymphalidae: Ithomiini): Comparative models for mimicry genetic studies. Molecular Ecology Resources, 23(4), 872–885. <https://doi.org/10.1111/1755-0998.13749>.

GBIF.org (n.d.). GBIF occurrence download. [Dataset]. <https://doi.org/10.15468/dl.6zagkz>.
...
Gu, J., Stevens, S., Campolongo, E.G., Thompson, M.J., Zhang, N., Wu, J., et al. (2025). BioCLIP 2: Emergent Properties from Scaling Hierarchical Contrastive Learning. arXiv:2505.23883v2. <https://doi.org/10.48550/arXiv.2505.23883>.
...
Imageomics (n.d.). BioCLIP 2.5 Huge. [Model card and model resources]. <https://huggingface.co/imageomics/bioclip-2.5-vith14>. Accessed 22 September 2026.

Karger, D.N., Conrad, O., Böhner, J., Kawohl, T., Kreft, H., Soria-Auza, R.W., et al. (2021). Climatologies at high resolution for the earth’s land surface areas. CHELSA version 2.1 [Dataset]. EnviDat. <https://doi.org/10.16904/envidat.228>.

Kass, J.M., Muscarella, R., Galante, P.J., Bohl, C.L., Pinilla-Buitrago, G.E., Boria, R.A., et al. (2021). ENMeval 2.0: Redesigned for customizable and reproducible modeling of species’ niches and distributions. Methods in Ecology and Evolution, 12(9), 1602–1608. <https://doi.org/10.1111/2041-210X.13628>.

MapLibre Contributors (n.d.). MapLibre GL JS. [Documentation]. <https://maplibre.org/maplibre-gl-js/docs/>. Accessed 22 September 2026.

McClure, M., Clerc, C., Desbois, C., Meichanetzoglou, A., Cau, M., Bastin-Héline, L., et al. (2019). Why has transparency evolved in aposematic butterflies? Insights from the largest radiation of aposematic butterflies, the Ithomiini. Proceedings of the Royal Society B: Biological Sciences, 286(1901), 20182769. <https://doi.org/10.1098/rspb.2018.2769>.
...
Phillips, S.J., Dudík, M., Elith, J., Graham, C.H., Lehmann, A., Leathwick, J., et al. (2009). Sample selection bias and presence-only distribution models: implications for background and pseudo-absence data. Ecological Applications, 19(1), 181–197. <https://doi.org/10.1890/07-2153.1>.

Python Software Foundation (n.d.). The Python Language Reference. <https://docs.python.org/3/reference/>. Accessed 22 September 2026.
...
Rosser, N. & Mallet, J. (2024). Interactive maps for visualizing geographic distributions and phenotypes. Tropical Lepidoptera Research, 34(2), 104–107. <https://mallet.oeb.harvard.edu/publications/interactive-maps-visualizing-geographic-distributions-and-phenotypes>.

Ultralytics (n.d.). Ultralytics YOLO Docs. <https://docs.ultralytics.com/>. Accessed 22 September 2026.
...
van der Heijden, E.S.M., Näsvall, K., Seixas, F.A., Beserra Nobre, C.E., Maia, A.C.D., Salazar-Carrión, P., et al. (2025). Genomics of Neotropical biodiversity indicators: Two butterfly radiations with rampant chromosomal rearrangements and hybridization. Proceedings of the National Academy of Sciences, 122(31), e2410939122. <https://doi.org/10.1073/pnas.2410939122>.

Wahlberg, N. (n.d.). The Nymphalidae Systematics Group. <https://www.nymphalidae.net/>. Accessed 22 September 2026.

Warren, A.D., Davis, K.J., Stangeland, E.M., Willmott, K.R. & Grishin, N.V. (2024). Illustrated Lists of American Butterflies. Version 9 March 2024. Butterflies of America. <https://www.butterfliesofamerica.com/L/Neotropical.htm>. Accessed 22 September 2026.
...
Wisz, M.S., Hijmans, R.J., Li, J., Peterson, A.T., Graham, C.H., Guisan, A., et al. (2008). Effects of sample size on the performance of species distribution models. Diversity and Distributions, 14(5), 763–773. <https://doi.org/10.1111/j.1472-4642.2008.00482.x>.

You, E. (n.d.). Vue.js: The Progressive JavaScript Framework. <https://vuejs.org/>. Accessed 22 September 2026.
