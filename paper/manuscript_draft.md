...
Turning that opportunity into practice, however, requires solving several practical problems. Researchers working with Ithomiini specimens typically photograph thousands of preserved wings, then read a handwritten identifier label from each photograph and rename the file accordingly. Renamed images go to cloud storage but remain scattered across folders with no browsing interface. Researchers must then manually combine geographic data from published records, institutional databases, GBIF, and iNaturalist, clean taxonomic inconsistencies, and map the results using GIS software. Each step relies on different tools, introduces delays, and creates opportunities for errors.

Early attempts to share images through R-Shiny applications hosted on free cloud instances ran into practical limitations: IP-based URLs proved hard to share, and free DNS services that provided custom domains were blocked by internet service providers in some countries, leaving the applications unreachable for collaborators. These experiences led us to GitHub Pages, which provides free, reliable static hosting without such dependencies.

Interactive web-based maps have begun to change how researchers explore biodiversity data. Rosser & Mallet (2024) built open-source interactive maps for *Heliconius* butterflies using R and the Leaflet library, hosted on GitHub. Their platform lets researchers visualize phenotypic and geographic data in a single interface, and their explicit goal of making their methods "straightforward for researchers to adapt to their own taxa" inspired this work. However, the R/Leaflet approach has limitations: filter changes can be slow with large datasets, the interface is constrained by what R-Shiny or static HTML allows, and the platform covers only the visualization step. The earlier steps of specimen digitization, image management, and multi-source data integration remain outside its scope.
...
1. **AI Photo Processor** -- A desktop application that uses Google's Gemini AI to read handwritten specimen identifiers from wing photographs in batch, enabling automated file renaming at scale. While built for entomological collections, researchers can customize the AI prompt to extract other types of information from images, making it adaptable to different research contexts.
...
Large-scale specimen photography generates thousands of image files with generic camera-assigned filenames. Each specimen's wings are typically photographed on both sides (dorsal and ventral), with a handwritten identifier label placed alongside them. Manually reading these identifiers and renaming each file is slow and error-prone, especially when processing hundreds or thousands of images.
...
The mimicry ring lookup from Dore et al. (2023) contains 44 distinct mimicry ring categories (Table S1). The most record-rich rings are Agnosia (14,930 records across 83 species), Hermias (12,393 records, 50 species), Lerida (12,262 records, 65 species), and Mamercus (11,452 records, 58 species). The pipeline propagates these mimicry ring values to Sanger Institute and GBIF records through species and subspecies matching, allowing researchers to filter and visualize mimicry ring distributions across all data sources.
...
We thank Neil Rosser and James Mallet for developing the *Heliconius* maps platform, which inspired the design of Ithomiini Maps, and the Global Biodiversity Information Facility (GBIF) and its data publishers for providing open occurrence data. AI coding assistants were used during software development, including OpenAI ChatGPT Codex, Google Gemini, and Anthropic Claude Code. [Additional acknowledgments to be added.]
