...
def load_occurrence_records():
    ...
    all_records = []
    for json_file in ['map_points_sanger.json', 'map_points_dore.json']:
...
def clean_coordinates(df):
    ...
    df = df[
        (df['lng'] >= extent['west']) & (df['lng'] <= extent['east']) &
        (df['lat'] >= extent['south']) & (df['lat'] <= extent['north'])
    ]
...
def main():
    ...
    stats_df = pd.DataFrame(species_stats).sort_values('thinned_records', ascending=False)
